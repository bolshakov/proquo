...
describe("effectiveSize", () => {
    ...
    it("returns zeros for an empty PR", () => {
        expect(effectiveSize([], DEFAULT_CONFIG)).toEqual({effectiveLines: 0, excludedLines: 0, excludedFiles: 0});
    });
    ...
    it("down-weights a test file per the default 0.5 test-file weight", () => {
        ...
        // 40 * 1 + 60 * 0.5 = 70
        expect(result).toEqual({effectiveLines: 70, excludedLines: 0, excludedFiles: 0});
    });
    ...
    it("rounds the weighted sum to the nearest whole line", () => {
        ...
        expect(result).toEqual({effectiveLines: 2, excludedLines: 0, excludedFiles: 0});
    });
});
