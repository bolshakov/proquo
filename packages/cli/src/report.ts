...
import {fileSpreadNote as coreFileSpreadNote, footnoteParts, formatNumber, formatRange, SPLIT_NUDGE_BODY, SPLIT_NUDGE_LEAD, tierTail} from "@proquo/core";
...
function tierLine(effectiveLines: number, price: Price): string {
    ...
    return `${n} effective lines — about ${range} of focused review${tierTail(price.tier)}`;
}
